...
v1.1 make use of the ownerid given in the link
*/
...
function dwellings_plunder_getmoduleinfo(){
	$info = array(
		...
		"requires"=>array(
			"dwellings"=>"20061019|Dwellings Project Team,http://dragonprime.net/users/sixf00t4/dwellingspack.zip",
		),
		"settings"=>array(
	...
	switch ($hookname){
		...
		case "dwellings-list-interact":
			...
			if ($session['user']['location'] == $args['location']
				...
				if ($args['owner']==$session['user']['acctid']) break;
...
function dwellings_plunder_run(){
	...
	if ($session['user']['specialmisc']=='') { //easier way to prevent unnecessary queries
		...
		if (!is_array($plunder)) $plunder=plunder_getmemyarray($dwid);
		//reload if no array
	}
						...
						case 2:
							...
							if ($damage>=$session['user']['hitpoints']) {
								...
								if ($plunder['name']!='')
									...
									addnews("%s`^ tried to plunder a nameless dwelling but ran into a trap and got killed!",$session['user']['name']);
								$session['user']['specialmisc']='';
								addnav("Return");
		...
		case "fight":
			if (httpget('mode')=='setup') {
				$badguy = array(
					...
					"creatureattack"=>round($plunder['attack']*1.05,0),
					"creaturedefense"=>round($plunder['defense']*1.25,0), //hard fighter
					"creaturehealth"=>$plunder['maxhitpoints']+e_rand(1,$plunder['maxhitpoints']),
					"diddamage"=>0,
				);
			...
			if ($victory){ //no exp at all for such a foul act
				if ($plunder['name']!='')
					...
					addnews("%s`^ has somehow survived a failed plunder at a nameless dwelling!",$session['user']['name']);
				...
				$session['user']['badguy']="";
				if (e_rand(0,1)) {
					output("`n`n`5%s`5 is critically wounded and falls down unconcious... yet it seems nobody woke up... excellent... you can continue on your plunder tour...",$plunder['residentname']);
					addnav("Continue to the dwelling","runmodule.php?module=dwellings_plunder&op=commencetheplunder&mode=inside");
				} else {
				...
				if ($plunder['name']!='')
					...
					addnews("%s`^ has been killed by %s`^ while trying to plunder a nameless dwelling!",$session['user']['name'],$plunder['residentname']);
				require_once("lib/systemmail.php");
