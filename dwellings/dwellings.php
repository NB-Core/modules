...
function dwellings_getmoduleinfo() {
	...
	// Added this setting  translation / bug fix. See: run/case_enter.php for explanation.
...
function dwellings_install(){
	global $session;
	require("modules/dwellings/install.php");
	return true;
}
...
function dwellings_run() {
	...
	if ($op != "list" && $op != ""){
		...
		addnav("Village"); // To form with other navs
		if ($session['user']['location']==get_module_setting('logoutlocation')) $session['user']['location']=get_module_pref("location_saver");
		villagenav();
		addnav("Navigation");
	}
	if($type == "" && $dwid>0){
