<?php
	...
	switch (httpget("subop")){
			...
			if (!$sub){
				...
				if (!$fail){
					...
					if (($gold_paid+$gold) >= $gold_cost && ($gems_paid+$gems) >= $gems_cost){
						output("`#Congratulations!`n`n");
						output("`3Your dwelling is now equipped with it's own personal guard.`n");
						output("This guard shall run out in `b%s`b days and you will be notified of its dismissal.",$days);
		...
		case "guardpreview":
			...
			output("This service costs `^%s `3gold and `%%s `3gems.`n`n",$gold_cost,$gems_cost);
			addnav("Buy a guard");
			addnav(array("`\$Yes (`^%s gold`\$,`%%%s gems`\$)",$gold_cost,$gems_cost),"runmodule.php?module=dwellings_pvp&op=manage&subop=rehire&dwid=$dwid&type=$typeid");
			break;
		case "renew": case "rehire":
			output("`#Congratulations!`n");
			output("`3Your dwelling is now equipped with it's own personal guard.`n");
			output("This guard shall run out in `b%s`b days and you will be notified of it's dismissal.`n",$days);
