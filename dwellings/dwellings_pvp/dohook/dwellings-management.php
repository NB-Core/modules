	...
	if (get_module_objpref("dwellingtypes",$typeid,"buy-guard","dwellings_pvp")){
		...
		if (!get_module_objpref("dwellings",$dwid,"bought","dwellings_pvp")){
			if($session['user']['gold'] >= $gold_cost-$gold_paid && $session['user']['gems'] >= $gems_cost-$gems_paid){ 
				...
				rawoutput("[<a href='runmodule.php?module=dwellings_pvp&op=manage&subop=guardpreview&dwid=$dwid&type=$typeid'>$hirenew</a>].</td></tr>");
				addnav("","runmodule.php?module=dwellings_pvp&op=manage&subop=guardpreview&dwid=$dwid&type=$typeid");
			}else{
