<?php
	...
	$sql = "SELECT name FROM ".db_prefix("accounts")." WHERE acctid='$oldacctid'";
	$res = db_query($sql);
	if (db_num_rows($res)>0) {
		$row = db_fetch_assoc($res);
		$oldowner = $row['name'];
	} else {
		$oldowner = translate_inline('Long-gone-Mate');
	}
	...
	if (db_num_rows($res)>0) {
		...
		//there are no dwellings for the user to delete... stop here
		return $args;
	}
	...
	if(get_module_setting("delete","dwellings")==0){//delete it
		...
		$sql = "SELECT dwid,location,type FROM ".db_prefix("dwellings")." WHERE ownerid=$oldacctid";
		$result = db_query($sql) or die(db_error(LINK));	
		while ($row = db_fetch_assoc($result)) { 
			$sql2 = "SELECT keyowner FROM ".db_prefix("dwellingkeys")." WHERE dwid='{$row['dwid']} AND dwidowner=$oldacctid AND keyowner != $oldacctid ORDER BY keyid DESC LIMIT 1";
		...
		$sql = "SELECT $accounts.marriedto AS marriedto, $dwellings.dwid AS dwid FROM $accounts INNER JOIN $dwellings ON $dwellings.ownerid = $accounts.acctid WHERE acctid=$oldacctid";
		$result = db_query($sql) or die(db_error(LINK));	 
		while ($row = db_fetch_assoc($result)){
			if ($row['marriedto']!=0 && $row['marriedto']!=INT_MAX) {//give to partner
				...
				$sql = "UPDATE ".db_prefix("dwellings")." SET ownerid = ".$row['marriedto']." WHERE dwid=".$row2['dwid']."";
				db_query($sql);
			} else{
