...
// We don't need this anymore, so we will free up the consumed memory.
	...
	if(!db_num_rows($result)){
		...
		while ($row = db_fetch_assoc($result)){
			$type = $row['type'];
			// $long_objpref = get_module_objpref("city",$cityid,"show$type",$type);
			// if ($long_objpref) continue;
	...
	rawoutput("</table>");
	// Show a list of all dwellings the user owns or has keys to and the respective locations in a new table
    $no_enter = translate_inline("You need to travel");
    ...
    $result = db_query($sql);
    rawoutput("<table border='0' cellpadding='3' cellspacing='0' align=center>");
    rawoutput("<tr class='trhead'><td style='width:100px;' nowrap>$tname</td><td style='width:100px;' nowrap>$towner</td><td style='width:100px;' nowrap>Location</td><td style='width:300px;'>$tdesc</td><td>$ttype</td><td>&nbsp;</td></tr>");
    if(!db_num_rows($result)){
        $none = translate_inline("None");
        rawoutput("<tr class='trdark'><td align=center colspan=5><i>$none</i></td></tr>");
    }else{
        $i = 0;
        while ($row = db_fetch_assoc($result)){
            $type = $row['type'];
            ...
            rawoutput("</td>");
            rawoutput("<td nowrap>[");
            output_notl($no_enter);
            rawoutput("]</td></tr>");
            $i++;
        }
    }
    rawoutput("</table>");
    // Now the same for other dwellings in other villages you own
