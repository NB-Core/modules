<?php
$dwid = (int) $dwid;
if (empty($dwid) || !is_numeric($dwid) || $dwid <= 0) {
	$dwid = (int)get_module_pref("dwelling_saver", "dwellings");
	if (empty($dwid) || !is_numeric($dwid) || $dwid <= 0) {
		output("`4You seem to have lost your way. Returning you to the village...`0");
		debuglog("$dwid - dwid was not set right, back to village");
...
// let's re-fetch the type if the passed dwid was wrong
if($type == "" && $dwid>0){
...
addcommentary();
$tl = get_module_objpref("dwellings",$dwid,"dwidtalkline","dwellings");
