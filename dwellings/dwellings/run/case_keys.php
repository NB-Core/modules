	...
	case "":
		...
		for ($i=0;$i<$maxkeys;$i++){
			$row = db_fetch_assoc($result);
                	if(isset($row) && is_array($row) && $row['keyid']>0 && $row['keyowner']!=0){
		...
		if($confirm){
			...
			$sql = "SELECT $dwellingkeys.dwidowner AS dwidowner, 
				...
				$dwellings.location AS location
					FROM $dwellingkeys
					INNER JOIN $dwellings ON $dwellingkeys.dwid = $dwellings.dwid
			...
			// what mongrel did link above dwid = dwidowner? in what world does that make sense? of course the code below never worked...
			$result = db_query($sql);
			$row = db_fetch_assoc($result);
			if (db_num_rows($result)>0) {
				//technically an error case else
