			...
			if($dwid > 0){
				if($subop == ""){//value check then make payment on existing dwelling
					...
					if (($paidgems > $session['user']['gems'] ||
						$paidgold > $session['user']['gold']) && $allowpay == 1){
						$allowpay = 0;
						if($paidgems > $session['user']['gems']) {
					...
					if (($paidgems > $carrygems) || ($paidgold > $carrygold)){
						$allowpay = 1;
						if($paidgems > $carrygems) $paidgems = $carrygems;
						if($paidgold > $carrygold) $paidgold = $carrygold;
					}
					if($allowpay){
						$session['user']['gold']-=$paidgold;
						$session['user']['gems']-=$paidgems;
						debuglog(sanitize($session['user']['name'])." paid $paidgold gold and $paidgems gems onto dwelling no. $dwid");
...
//Changed this, so we can _always_ translate these
					...
					if($allowpay == 1){
						...
						$sql = "INSERT INTO ".db_prefix("dwellings")." (gold,gems,goldvalue,gemvalue,ownerid,type,location,status,description,windowpeer,storedinfo) VALUES ($paidgold,$paidgems,$goldvalue,$gemvalue,".$session['user']['acctid'].",'$type','".$session['user']['location']."',2,'',0,0)";
						$result = db_query($sql);
