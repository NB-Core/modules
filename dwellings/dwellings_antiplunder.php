...
function dwellings_antiplunder_dohook($hookname,$args){
	...
	switch ($hookname) {
		...
		case "dwellings-inside":
			...
			if ($guard) {
				$sql="SELECT name,weapon,sex FROM ".db_prefix("accounts")." WHERE acctid=$guard";
				$row=db_fetch_assoc(db_query($sql));			
				$pro=translate_inline($row['sex']?"her":"his");
				output("`5%s`5 stands guard at the coffers, prepared with %s %s`5.`n",$row['name'],$pro,$row['weapon']);
			}
