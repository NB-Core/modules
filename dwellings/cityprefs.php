...
function cityprefs_getmoduleinfo() {
	$info = array(
		...
		"download"=>"http://dragonprime.net/users/sixf00t4/cityprefs.zip",
    );                
	return $info;
}
...
function cityprefs_install(){
    ...
    if (!is_module_active('cityprefs')){
		...
		foreach($vloc as $loc=>$val) {
			$sql = "select modulename from ".db_prefix("module_settings")." where value='$loc' and setting='villagename'";
			$result=db_query($sql);
			$row = db_fetch_assoc($result);
			$sql = "INSERT INTO ".db_prefix("cityprefs")." (module,cityname) VALUES ('".$row['modulename']."','$loc')";
			db_query($sql);  
		}
    }else{
   ...
   if($op!="su") addnav("Back to city list","runmodule.php?module=cityprefs&op=su");
    switch ($op) {
        ...
        case "update":
            ...
            $out=0;
            foreach($vloc as $loc=>$val) {
                $sql = "select cityname from ".db_prefix("cityprefs")." where cityname='$loc'";
                $result=db_query($sql);
        ...
        case "editmodulesave":
			...
			//$area = httpget("area");
            //viewcommentary($area,"Speak into this city",25,"projects");
			}else{
