<?php
			...
			if ( get_module_setting("pvp")){
				...
				if ($al > $goodalign && $ual < $evilalign){
					...
					output("`n`bYou have smitten a good person, and as you are evil... it makes you more evil.`b`0`n");
			...
			if (get_module_setting("de-pvp")){
				$ual = get_module_pref("demeanor");
				$al = get_module_pref("demeanor","alignment",$badguy['acctid']);
				if ($al > $lawalign && $ual < $chaosalign){
					$new = round($session['user']['level']/2);
					output("`n`bYou have smitten a lawful person, and as you are chaotic... it makes you more chaotic.`b`0`n");
					require_once("modules/alignment/func.php");
					demeanor("-$new");
			...
			if (isset($args['type']) && ($args['type'] == 'forest' || $args['type'] == 'travel')){
				$id = $badguy['acctid'];
