...
function halloween_getmoduleinfo(){
	$info = array(
		"name"=>"`qHallo`Qween `~Surprise`0 (timelocked)",
		"version"=>"1.0",
...
function halloween_dohook($hookname,$args){
	...
	switch ($hookname) {
		case "newday":
			...
			break;
		case "findloot-categories":
			$args['categories'].=",Halloween";
			return $args;
			break;
		case "pvpmodifytargets":
			$id=$session['user']['acctid'];
			$loc=isset($args[0]['location'])?$args[0]['location']:"";
	...
	//			"AND (laston<'$last' OR loggedin=0) AND (acctid<>$id) " . 
		...
		case "pvpadjust":
			if (e_rand(0,9)==7) { //pumpkin massacre, check pvpsupport and pvplist for locationblockers as well as pvplist for "cannot reach"
				$row=$args;
				$sql = "UPDATE " . db_prefix("accounts") . " SET pvpflag='".DATETIME_DATEMIN."' WHERE acctid={$row['acctid']}";
				db_query($sql);
				$row['creaturelevel']=$session['user']['level'];
				$row['acctid']=7; //let the original player live
